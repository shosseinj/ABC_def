...
def stage_a_gate(rows):
    ...
    criteria={"criterion1_aggregate_quantum_positive":bool(agg is not None and agg>0),"criterion2_quantum_positive_splits":sum(q[s]["value"] is not None and q[s]["value"]>0 for s in SPLIT_SEEDS)>=3,"criterion3_margin_and_spearman":sum(m[s]["value"] is not None and m[s]["value"]>0 for s in SPLIT_SEEDS)>=3 and rho["rho"] is not None and rho["rho"]>0,"criterion4_all_single_id_exclusions":bool(loo_rows) and all(r["all_required_directions_hold"] for r in loo_rows)}
    qci=None
    if len(qvals)==5:
        half=2.776*float(np.std(qvals,ddof=1))/np.sqrt(5);qci=[float(agg-half),float(agg+half)]
    return {"pass":all(criteria.values()),"criteria":criteria,"leave_one_id_out":loo_rows,"aggregate_quantum_l2_contrast":agg,"aggregate_quantum_l2_descriptive_t4_ci95":qci,"aggregate_status":st,"split_quantum_l2":q,"split_margin_drop":m,"spearman":rho,"top_contributor_id":top[1],"top_contributor_absolute_aggregate_change":top[0],"top_contributor_excluded_aggregate":top[2],"top_contributor_excluded_split_values":dict(zip(SPLIT_SEEDS,top[3])),"top_contributor_excluded_positive_splits":loo_positive,"implementation":"all eligible canonical single-ID exclusions must retain at least 3 positive split directions; top contributor retained descriptively only"}
